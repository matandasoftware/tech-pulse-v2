...
from .serializers import (ArticleReferenceSerializer, NoteSerializer,
                          ReadingHistorySerializer, UserArticleSerializer)
...
class UserArticleListView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return UserArticle.objects.filter(user=self.request.user).select_related(
...
class BookmarkedArticlesView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            UserArticle.objects.filter(user=self.request.user, is_bookmarked=True)
...
class ReadArticlesView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            UserArticle.objects.filter(user=self.request.user, is_read=True)
            .select_related("article__source", "article__category")
            .order_by("-read_at")
        )
...
class SavedForLaterView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            UserArticle.objects.filter(user=self.request.user, is_saved_for_later=True)
            .select_related("article__source", "article__category")
            .order_by("-saved_at")
        )
...
class PendingFollowUpsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            Note.objects.filter(
                user=self.request.user, has_follow_up=True, follow_up_done=False
            )
            .select_related("article__source", "article__category")
...
class ReadingHistoryListView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return ReadingHistory.objects.filter(user=self.request.user).select_related(
            "article__source", "article__category"
        )
...
def dashboard_stats(request):
    ...
    pending_followups = Note.objects.filter(
        user=user, has_follow_up=True, follow_up_done=False
    ).count()

    completed_followups = Note.objects.filter(
        user=user, has_follow_up=True, follow_up_done=True
    ).count()

    # Total articles available in the system (not just user's interactions)
    from articles.models import Article

    total_articles = Article.objects.count()

    # User's interacted articles
    user_articles = (
        UserArticle.objects.filter(user=user).values("article").distinct().count()
    )
    ...
    notes_timeline = (
        Note.objects.filter(user=user, created_at__gte=thirty_days_ago)
    ...
    overdue_followups = Note.objects.filter(
        user=user, has_follow_up=True, follow_up_done=False, follow_up_date__lt=today
    ).count()

    # Due today follow-ups
    due_today_followups = Note.objects.filter(
        user=user, has_follow_up=True, follow_up_done=False, follow_up_date=today
    ).count()

    # Upcoming follow-ups count (future dates)
    upcoming_followups_count = Note.objects.filter(
        user=user, has_follow_up=True, follow_up_done=False, follow_up_date__gt=today
    ).count()
    ...
    return Response(
        {
            "counts": {
                "total_articles": total_articles,  # Total articles in system
                "user_articles": user_articles,  # Articles you've interacted with
            ...
            "top_sources": [
                {"name": item["article__source__name"], "count": item["count"]}
                for item in top_sources
                if item["article__source__name"]
            ],
            "top_categories": [
                {"name": item["article__category__name"], "count": item["count"]}
                for item in top_categories
                if item["article__category__name"]
            ],
            "notes_timeline": [
