...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if count > 0:
            self.stdout.write(
                f"Found {count} unbookmarked articles with bookmarked_at timestamp"
            )
            unbookmarked_with_timestamp.update(bookmarked_at=None)
            self.stdout.write(
                self.style.SUCCESS(f"✓ Cleared {count} bookmarked_at timestamps")
            )

        # Fix read status
        unread_with_timestamp = UserArticle.objects.filter(is_read=False).exclude(
        ...
        if count > 0:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"✓ Cleared {count} read_at timestamps")
            )
        ...
        if count > 0:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"✓ Cleared {count} saved_at timestamps")
            )
