...
from articles.models import Source, Category, Article
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for source in sources:
            ...
            if not options['force'] and source.last_fetched:
                ...
                if time_since_fetch < timedelta(minutes=source.fetch_frequency):
                    self.stdout.write(self.style.WARNING(
                        f'  Skipping (last fetched {time_since_fetch.seconds // 60} minutes ago)'
                    ))
                    continue
            
            if not source.rss_feed_url:
                self.stdout.write(self.style.WARNING('  No RSS feed URL configured'))
                continue
            ...
            try:
                ...
                self.stdout.write(self.style.SUCCESS(f'  Created {created_count} new articles'))
                
            except Exception as e:
                self.stdout.write(self.style.ERROR(f'  Error fetching feed: {e}'))
        
        self.stdout.write(self.style.SUCCESS(
            f'\n✅ Fetch complete! Processed {total_fetched} entries, created {total_created} new articles'
        ))
    ...
    def _determine_category(self, title, content, entry):
        ...
        # Category keywords
        categories_keywords = {
            'Artificial Intelligence': ['ai', 'artificial intelligence', 'machine learning', 'deep learning', 'neural network', 'gpt', 'llm'],
            'Cybersecurity': ['security', 'hack', 'breach', 'vulnerability', 'cybersecurity', 'malware', 'ransomware'],
            'Cloud Computing': ['cloud', 'aws', 'azure', 'google cloud', 'serverless', 'kubernetes'],
            'Mobile': ['mobile', 'iphone', 'android', 'ios', 'smartphone', 'app store'],
            'Programming': ['programming', 'code', 'developer', 'javascript', 'python', 'github'],
            'Hardware': ['hardware', 'processor', 'chip', 'cpu', 'gpu', 'semiconductor'],
